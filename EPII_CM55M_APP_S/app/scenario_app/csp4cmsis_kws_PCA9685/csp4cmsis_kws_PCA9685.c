...
// (QUARTER_SECOND_MONO_BYTES / BLK_NUM / AUDIO_LEN / NUM_BUFF now come from csp4cmsis_kws_pdm_record.h)
...
struct_kws_algoResult algoresult_kws_pdm_record;
...
volatile int32_t w_buf_idx = 0;
volatile int32_t r_buf_idx = 0;

// Ground-truth measurement of the ACTUAL DMA completion callback interval,
// independent of any assumption about what BLK_NUM does. AcquisitionProcess
// reports these periodically (see kws_report_acq_stats caller) so we can see
// directly whether the callback is firing every ~250ms or ~500ms in real
// wall-clock time, rather than inferring it from driver-semantics guesses.
volatile uint32_t g_dma_cb_count = 0;
volatile uint32_t g_dma_cb_interval_accum_ms = 0; // sum of inter-callback gaps since last read
volatile uint32_t g_dma_cb_interval_count = 0;
static volatile uint32_t g_dma_cb_last_tick = 0;
...
int kws_pdm_record_app(void)
{
    ...
    {
        case PDM_PCM_FREQ_8K:  srate = 8000;  break;
        case PDM_PCM_FREQ_16K: srate = 16000; break;
        case PDM_PCM_FREQ_32K: srate = 32000; break;
        case PDM_PCM_FREQ_48K: srate = 48000; break;
        default:               srate = 16000; break;
    }
    num_chans = 1;
