 ...
 * updated incrementally by 25 frames (one 0.25 s quarter-buffer's worth) per
 ...
 *
 * Frame arithmetic: one quarter-buffer is 4000 samples (0.25 s @ 16 kHz).
 * With frameLength=480 and frameStride=160, that yields exactly
 * (320 + 4000 - 480) / 160 + 1 = 25 new frames per step, and 4000/160 = 25
 * exactly -- no remainder in either case, confirming the quarter-buffer sizing
 ...
 * the END of the PREVIOUS quarter-buffer; it is carried explicitly in
...
namespace {
    ...
    constexpr int32_t kPPNewSamplesPerStep = 4000;                            // 0.25 s @ 16 kHz
    constexpr int32_t kPPNewFramesPerStep  = 25;                              // (320+4000-480)/160 + 1
    constexpr int32_t kPPWindowSamples     = kPPTailSamples + kPPNewSamplesPerStep; // 4320

    int16_t g_pp_tail[kPPTailSamples];
    // Counts up across the first 4 steps after (re)initialisation, then keeps
...
static void ShiftAndAppendFrames(const int16_t* window4320) {
    const int32_t oldRows = kNumRows - kPPNewFramesPerStep; // 73
...
void* cv_kws_preprocess_step(const int16_t *newQuarterBuffer) {
    static int16_t scratch[kPPWindowSamples];   // 4320 samples; static, not on the task's own stack
