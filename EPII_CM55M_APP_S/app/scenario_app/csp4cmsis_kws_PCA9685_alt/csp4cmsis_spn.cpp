...
#include "hx_drv_uart.h"   // hx_drv_uart_get_dev() / DEV_UART_PTR -- for console keypress input
}
...
struct KwsReportMsg {
    enum class Kind : uint8_t {
        ...
        PcaCommand   // NEW: PCA9685 servo move, from either voice or console
    } kind;
    char    label[16];
    int32_t  scorePct;
    uint32_t labelIdx;
...
// Second, independent command source: raw WASD keypresses from
// ConsoleInputProcess. Arbitrated against g_hwCmdChan via Alternative
// inside Pca9685Process, same pattern as tests.cpp's Receiver.
static Channel<KwsTokenMsg> g_consoleCmdChan;
...
// storage (no more implicit fallback, per the v1.3 API notes above
// InferenceProcess/PreprocessingProcess). CSProcessStatic<N> is the library
// helper that supplies that storage for you -- inherit from it instead of
// CSProcess directly and give it the stack size in words. Sizes below are
// provisional (same "measure the HWM report line, then right-size" caveat
// as MAIN_APP_STACK_WORDS); ReporterProcess just does xprintf formatting so
// 1024 words is a generous starting point.
        ...
        while (true) {
		    ...
		    break;
                ...
                case KwsReportMsg::Kind::MemStats:
                    ...
                    break;
                case KwsReportMsg::Kind::PcaCommand: {
                    // Field reuse (matches the rest of this struct's style):
                    // label=position name, labelIdx=source, a=channel,
                    // prevBuf=newPos index, total=max index, missed=atLimit flag.
                    const char *source = (msg.labelIdx == 1) ? "key"
                                        : (msg.labelIdx == 2) ? "init" : "voice";
                    xprintf("[PCA9685/%s] Ch%ld -> %s (%ld/%ld)%s\r\n",
                            source, (long)msg.a, msg.label, (long)msg.prevBuf, (long)msg.total,
                            msg.missed ? " [at limit]" : "");
                    break;
                }
            }
...
/*******************************************************************************
 * ConsoleInputProcess: second, independent PWM command source. Polls the
 * console UART for WASD keypresses and turns them into the same
 * up/down/left/right vocabulary the voice path uses, sent straight to
 * Pca9685Process -- deliberately bypassing FilterProcess/FsmProcess. A
 * keypress is an unambiguous, deliberate action (unlike a probabilistic
 * KWS detection), so it doesn't need the voice path's "direction, then
 * go" confirmation step; it should move the servo immediately.
 ******************************************************************************/
// Just a poll loop + a tiny switch, minimal stack needed.
class ConsoleInputProcess : public CSProcessStatic<512> {
    Chanout<KwsTokenMsg> out;
    DEV_UART_PTR uart_dev = nullptr;

    // TODO: confirm USE_DW_UART_0 is actually the console/debug UART on
    // your board -- this file has no visibility into which instance
    // xprintf's own backend binds to. UART_BAUDRATE_115200 is safe to
    // pass either way it turns out: per hx_drv_uart.h, if the device is
    // already open (which it will be by the time this task starts,
    // since the console is already in use for TX), uart_open() just
    // bumps a refcount and leaves the existing baud/config untouched.
    static constexpr USE_DW_UART_E kConsoleUartId = USE_DW_UART_0;
...
public:
    ...
    void run() override {
        ...
        if (uart_dev != nullptr) {
            uart_dev->uart_open(UART_BAUDRATE_115200);
        }
        ...
        while (true) {
            ...
            switch (c) {
                case 'w': case 'W': label = "up";    break;
                case 's': case 'S': label = "down";  break;
                case 'a': case 'A': label = "left";  break;
                case 'd': case 'D': label = "right"; break;
                default: continue; // ignore anything else, incl. CR/LF/echo noise
            }
...
class Pca9685Process : public CSProcessStatic<1024> {
    Chanin<bool> i2c_sync;
    Chanin<KwsTokenMsg> in_hw;       // confirmed voice commands, via FsmProcess
    Chanin<KwsTokenMsg> in_console;  // raw WASD keypresses, via ConsoleInputProcess
    Chanout<KwsReportMsg> report_out;
    ...
    // Replaces the direct xprintf calls this task used to make. This task
    // is now also the Alternative point for time-sensitive keypress
    // input (see run() below), so it must not touch the console UART
    // itself -- TX stays exclusively owned by ReporterProcess/g_reportChan,
    // RX by ConsoleInputProcess. source: 0=voice/FSM, 1=console key, 2=init.
    void report_position(uint8_t channel, int newPos, bool atLimit, int source) {
        KwsReportMsg msg{};
        msg.kind = KwsReportMsg::Kind::PcaCommand;
        std::strncpy(msg.label, kPositionNames[newPos], sizeof(msg.label) - 1);
        msg.labelIdx = (uint32_t)source;
        msg.a = channel;
        msg.prevBuf = newPos;
        msg.total = kNumPositions - 1;
        msg.missed = atLimit ? 1 : 0;
        report_out << msg;
    }
    ...
    void init() {
        ...
        report_position(0, position[0], false, /*source=*/2);
        report_position(1, position[1], false, /*source=*/2);
    }

    // Shared by both command sources: up/down step channel 0, left/right
    // step channel 1. "up" and "left" both step towards Max Left (-1);
    // "down" and "right" both step towards Full Right (+1).
    void apply_command(const KwsTokenMsg &cmd, int source) {
        ...
        if (channel < 0) {
            return; // neither source should ever send anything else, but be defensive
        }
        ...
        report_position((uint8_t)channel, newPos, atLimit, source);
    }
...
public:
    ...
    void run() override {
        ...
        // Same pattern as tests.cpp's Receiver: fairSelect() arbitrates
        // between the two sources so a held key can't starve a pending
        // voice command, or vice versa.
        Alternative alt(in_hw | voiceCmd, in_console | consoleCmd);

        while (true) {
            int selected = alt.fairSelect();
            if (selected == 0) {
                apply_command(voiceCmd, /*source=*/0);   // confirmed FSM/voice command
            } else {
                apply_command(consoleCmd, /*source=*/1); // console keypress
            }
        }
    }
};
...
void MainApp_Task(void* params) {
    ...
    // New: reads WASD keypresses off the console, second command source
    static ConsoleInputProcess  console_in(g_consoleCmdChan.writer());

    // PCA9685 process: ISR sync channel, the two command sources (FSM +
    // console, arbitrated internally via Alternative), and the report
    // channel it now uses instead of calling xprintf directly.
    static Pca9685Process       pca9685_hw(g_pca9685_i2c_isr_chan.reader(),
                                            g_hwCmdChan.reader(),
                                            g_consoleCmdChan.reader(),
                                            g_reportChan.writer());
